...
(function($, undefined) {
  ...
  var debug = true,
    options = {
      mouseport: "body", // $ object or selector of DOM node to use as mouseport
      xparallax: true, // boolean | 0-1 | 'npx' | 'n%'
      yparallax: true, //
      xorigin: center, // 0-1 - Sets default alignment. Only has effect when parallax values are something other than 1 (or true, or '100%')
      yorigin: center, //
      decay: 0.1, // 0-1 (0 instant, 1 forever) - Sets rate of decay curve for catching up with target mouse position
  ...
  var Timer = (function() {
    ...
    Timer.prototype = {
      add: function(fn) {
        ...
        if (debug) {
          console.log("timer add()", this.callbacks.length);
        }
      },
      ...
      remove: function(fn) {
        ...
        if (debug) {
          console.log("timer remove()", this.callbacks.length);
        }
  ...
  function getOffset(elem) {
    var offset = elem.offset() || { left: 0, top: 0 },
      borderLeft =
        elem.css("borderLeftStyle") === "none"
          ? 0
          : parseInt(elem.css("borderLeftWidth"), 10),
      borderTop =
        elem.css("borderTopStyle") === "none"
          ? 0
          : parseInt(elem.css("borderTopWidth"), 10),
      paddingLeft = parseInt(elem.css("paddingLeft"), 10),
      paddingTop = parseInt(elem.css("paddingTop"), 10);

    return [
      offset.left + borderLeft + paddingLeft,
    ...
    while (i--) {
      position[i] = px[i]
        ? // TODO: reverse calculation for pixel case
          0
        : pos[i === 0 ? "left" : "top"] /
          (viewport[i === 0 ? "outerWidth" : "outerHeight"]() - size[i]);
  ...
  function pointerOffTarget(
    ...
    if (
      (!parallax[0] ||
        Math.abs(targetPointer[0] - prevPointer[0]) < threshold[0]) &&
      (!parallax[1] ||
        Math.abs(targetPointer[1] - prevPointer[1]) < threshold[1])
    ) {
    ...
    while (x--) {
      if (parallax[x]) {
        pointer[x] =
          targetPointer[x] + decay * (prevPointer[x] - targetPointer[x]);
      }
    }
  ...
  function pointerOnTarget(
    ...
    if (
      targetPointer[0] === prevPointer[0] &&
      targetPointer[1] === prevPointer[1]
    ) {
      return;
    }
  ...
  $.fn.parallax = function(o) {
    var options = $.extend({}, $.fn.parallax.options, o),
      args = arguments,
      elem =
        options.mouseport instanceof $
          ? options.mouseport
          : $(options.mouseport),
    ...
    return this.each(function(i) {
      var node = this,
        elem = $(this),
        opts = args[i + 1] ? $.extend({}, options, args[i + 1]) : options,
        decay = opts.decay,
      ...
      port.elem.on(events);
      port.layers = port.layers ? port.layers.add(node) : $(node);
      ...
      /*$.event.add(this, 'freeze.parallax', freeze);
			$.event.add(this, 'unfreeze.parallax', unfreeze);*/
    });
  };

  $.fn.unparallax = function(bool) {
    return this.each(function() {
      ...
      $.removeData(this, "parallax");
      unparallax(this, data.port, data.events);
      if (bool) {
        unstyle(data.parallax);
      }
    });
  };

  $.fn.parallax.options = options;
